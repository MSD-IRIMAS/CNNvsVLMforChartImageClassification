...
def rectify_preds(Y_pred, Y_true, motif, reviewFilePath):
    ...
    for pred, true_class in zip(Y_pred, Y_true):
        ...
        if pred == "histogram":
            ...
            pred = "dot strip plot"
        elif (pred == "stacked bar" and true_class == "ordered bar"):
            pred = "bar"
...
def run_experiment(model_path, img_array_list, prompt_type):
    reviewFilePath = 'review/llava-v1.5-13b/Review_prompt_' + prompt_type + ".txt"
    prompt = None
    ...
    try:
        with open(reviewFilePath, 'w') as review:
            review.write('*******************************************************\n')
            review.write("             llava-v1.5-13b inference \n")
            review.write('*******************************************************\n')
