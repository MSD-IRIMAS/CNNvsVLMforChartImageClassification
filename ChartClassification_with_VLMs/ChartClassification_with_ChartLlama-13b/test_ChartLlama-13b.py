...
import numpy as np
import pandas as pd
from llava.eval.model_vqa_lora import load_pretrained_model
from llava.eval.model_vqa_lora import custom_inference
...
def run_experiment(model_path, model_base, img_array_list, prompt_type):
    ...
    for img_array in img_array_list:
        ...
        pred = custom_inference(img, prompt, model_path, model, tokenizer,
                                image_processor, temperature=0.2, max_new_tokens=30)
    ...
    try:
        with open(reviewFilePath, 'w') as review:
            review.write('*******************************************************\n')
            review.write("              ChartLlama-13b inference \n")
            review.write('*******************************************************\n')
...
if __name__ == '__main__':
    ...
    for pr in ["first", "second", "third", "fourth"]: # for each form of prompt
        preds_list, prompt_type, reviewFilePath = run_experiment(model_path, model_base, img_array_list, prompt_type=pr)
