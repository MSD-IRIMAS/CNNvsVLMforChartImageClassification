...
def matrice_confusion(y_true, y_predict, classes_name, prompt_type):
    print('Confusion matrix computing !')
    conf_matrix = metrics.confusion_matrix(y_true, y_predict, labels=classes_name)
    np.set_printoptions(precision=2)
    plt.figure(figsize=(15,15))
    plot_confusion_matrix(conf_matrix, classes=classes_name, title="paligemma-3b-ft-vqav2-448 confusion matrix")
    plt.savefig("review/Confusion_matrix_prompt_" + prompt_type + ".png")


def get_all_results(Y_true, Y_pred, classes_name, prompt_type, reviewFilePath):
    ...
    reportPath = 'review/Classif_report_prompt_' + prompt_type + '.csv'
    ...
    plt.grid(axis='y')
    plt.savefig('review/metrics_per_class_prompt_'+ prompt_type + '.png')

    matrice_confusion(Y_true, Y_pred, classes_name, prompt_type)
