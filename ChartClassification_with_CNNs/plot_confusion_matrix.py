...
def matrice_confusion(y_true, y_predict, classes_name, path_folder):
    print('Confusion matrix computing !')
    conf_matrix = metrics.confusion_matrix(y_true, y_predict, labels=classes_name)
    np.set_printoptions(precision=2)
    plt.figure(figsize=(15,15))
    plot_confusion_matrix(conf_matrix, classes=classes_name, title='Matrice de confusion')
    plt.savefig(path_folder)


def get_all_results(Y_true, Y_pred, classes_name, model_name, reviewFilePath, confMatPath):
    ...
    reportPath = 'saved_models/'+model_name+'_Classif_report.csv'
    ...
    plt.grid(axis='y')
    plt.savefig('saved_models/'+model_name+'_metrics_per_class.png')

    # matrice_confusion(Y_true, Y_pred, classes_name, confMatPath)
